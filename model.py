...
class Model(nn.Module):
    ...
    def get_loss_for_batch(self, batch: Batch, loss_function: nn.Module, nonreg_loss_function: nn.Module,
     xentloss_weight:int=0.0, regloss_weight:int=1.0, noise_rate:int = 0) \
        ...
        skel_out, nonreg_out = self.forward(
...
def build_model(cfg: dict = None,
                ...
                nonreg_trg_vocab: Vocabulary = None) -> Model:
    """
    ...
    ## text embedding
    nonreg_trg_embed = Embeddings(
        **cfg["encoder"]["embeddings"], vocab_size=len(nonreg_trg_vocab),#4146,
    ...
    encoder = TransformerEncoder(**cfg["encoder"],
                                 emb_size=cfg["encoder"]["embeddings"]["embedding_dim"],#src_embed.embedding_dim,
                                 emb_dropout=cfg["encoder"]["embeddings"]["dropout"])#enc_emb_dropout)
