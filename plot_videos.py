...
def writeAudio(vid_loc, audio_loc):
    ...
    cmd = "ffmpeg" + " -loglevel panic -i " + vid_loc + " -i " + "tmp.wav" #temporary .wav audio location
    cmd += " -c:v copy -c:a aac -strict experimental " + new_vid_loc
    os.system(cmd)
    os.remove("tmp.wav") # delete temporary .wav file
    os.remove(vid_loc)
    
    return new_vid_loc

# Plot a video given a tensor of joints, a file path, video name and references
def plot_video(joints,
    ...
    npy_folder = file_path.split('/')[1]+'/test_poses' # to store numpy files for output poses (optional)
    ...
    FPS = (30 // skip_frames)
    video_file = file_path + "{}.avi".format(video_name.split(".")[0])
    ...
    ## to save numpy file for output poses (optional)
    # np.save(os.path.join(npy_folder, video_name.split('.')[0]), np.stack((np.array(pred_kpts), np.array(gt_kpts)),0))
...
def draw_frame_2D(frame, joints):
    ...
    for j in range(number):
        ...
        draw_line(frame, [joints[skeleton[j, 0]][0], joints[skeleton[j, 0]][1]],
                  [joints[skeleton[j, 1]][0], joints[skeleton[j, 1]][1]], c=c, t=1, width=3)
